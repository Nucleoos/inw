...
#                        imp_valor += line[2]['valor_total_imp']
            ...
            if tot_trib_fed>0 or tot_trib_estadual>0 or tot_trib_municipal>0:
                ...
                result['total_municipal_valor'] = tot_trib_municipal
                result['total_municipal_perc'] = (tot_trib_municipal /total) * 100
                total_lic = total - ( tot_trib_fed + tot_trib_estadual)
                imp = ("Voce pagou aproximadamente:R$ %.2f de Tributos Federais, R$ %.2f de Tributos Estaduais, R$ %.2f Pelos Produtos Comercializados. Fonte:IBPT/FECOMERCIO " % (tot_trib_fed, tot_trib_estadual, total_lic)).replace('.',',').replace('*','.')
                result['comment'] = imp
